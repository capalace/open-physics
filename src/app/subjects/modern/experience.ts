...
class ModernController implements SubjectController {
  ...
  unmount(): void {
    this.renderer.destroy(); this.disposers.forEach((dispose) => dispose());
  ...
  private listen(element: Element, event: string, listener: EventListener): void { element.addEventListener(event, listener); this.disposers.push(() => element.removeEventListener(event, listener)); }
  private button(label: string, action: () => void): HTMLButtonElement { const result = document.createElement("button"); result.type = "button"; result.textContent = label; this.listen(result, "click", action); return result; }
  ...
  private guidedInspector(fragment: DocumentFragment): void {
    ...
    article.innerHTML = `<p class="modern-experience__category">${lab.category}</p><h2>${lab.title}</h2><p class="modern-experience__question">${lab.question}</p><ol>${lab.steps.map((step) => `<li>${step}</li>`).join("")}</ol><p class="modern-experience__observe"><strong>관찰:</strong> ${lab.observe}</p><div class="modern-experience__term"><strong>${lab.controls[0]}</strong></div><section class="modern-experience__law"><h3>${lab.law.title}</h3><p>${lab.law.description}</p><code>${lab.law.equation}</code></section><section class="modern-experience__graph"><h3>${lab.graph.title}</h3><p>${lab.graph.yLabel} / ${lab.graph.xLabel}</p></section>`;
  ...
  private sandboxInspector(fragment: DocumentFragment, snapshot: ModernSnapshot): void {
    ...
    const controls = document.createElement("div"); controls.className = "modern-experience__palette"; palette.forEach(({ kind, label }) => controls.append(this.button(`＋ ${label}`, () => { this.model.addDevice(kind); this.renderInspector(); this.refresh(); })));
    const list = document.createElement("ul"); list.className = "modern-experience__device-list"; snapshot.devices.forEach((item) => { const row = document.createElement("li"); row.textContent = palette.find(({ kind }) => kind === item.kind)?.label ?? item.kind; row.append(this.button("삭제", () => { this.model.removeDevice(item.id); this.renderInspector(); this.refresh(); })); list.append(row); });
