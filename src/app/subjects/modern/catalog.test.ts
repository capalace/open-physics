...
describe("modern-physics catalog", () => {
  it("contains all eight guided laboratories exactly once", () => {
  ...
  it("provides distinct direct controls, questions, and graphs", () => {
    expect(new Set(modernDefinition.labs.map((lab) => lab.controls[0])).size).toBe(8);
    expect(new Set(modernDefinition.labs.map((lab) => lab.question)).size).toBe(8);
    expect(new Set(modernDefinition.labs.map((lab) => lab.graph.title)).size).toBe(8);
