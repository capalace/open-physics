...
describe("thermal catalog", () => {
  it("contains each of the seven required guided labs exactly once", () => {
  ...
  it("gives every lab a unique manipulation and measurable graph", () => {
    expect(new Set(thermalDefinition.labs.flatMap((lab) => lab.controls)).size).toBe(7);
    expect(new Set(thermalDefinition.labs.map((lab) => lab.graph.title)).size).toBe(7);
